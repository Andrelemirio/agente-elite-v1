...
def webhook():
    ...
    try:
        ...
        # BUSCA VAGAS (COM FILTRO PARA EVITAR O ERRO DO LOG)
        cur.execute("SELECT hora FROM agenda WHERE disponivel=TRUE AND hora IS NOT NULL ORDER BY hora LIMIT 4")
        vagas_lista = [v[0] for v in cur.fetchall() if v[0] is not None] # Filtra nulos
        vagas_txt = ", ".join(vagas_lista) if vagas_lista else "Agenda lotada"

        resposta = ""
        nome_curto = nome.split()[0] if nome else "paciente"
        ...
        if estado == "TRIAGEM":
            ...
            resposta = f"Obrigado, {msg.split()[0]}. Para finalizar, digite apenas os 11 números do seu CPF."
            ...
            if len(cpf_limpo) != 11:
                ...
                resposta = f"Tudo pronto, {nome_curto}! Seu agendamento para {horario} foi confirmado com sucesso. Nossa equipe te aguarda!"
        else:
        ...
        print(f"❌ ERRO NO PROCESSO: {e}") # Isso vai mostrar o erro exato no log se acontecer
    finally:
        if conn: conn.close()
    return "OK", 200
...
@app.route('/')
def home(): return "🚀 AGENTE V6.3 BLINDADO ONLINE", 200
